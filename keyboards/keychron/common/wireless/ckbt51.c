...
void ckbt51_send_cmd(uint8_t* payload, uint8_t len, bool ack_enable, bool retry) {
    ...
    for (i = 0; i < 3; i++) {
        gpio_write_pin(MCU_TO_WIRELESS_INT_PIN, i % 2);
...
void ckbt51_connect(uint8_t hostIndex, uint16_t timeout) {
    ...
    ckbt51_send_cmd(payload, i, true, false);
    // Fix issue: sometimes module dosen't reconnect after host switching
    if (hostIndex != 0) {
        wait_ms(100);
        payload[2] = 0;
        ckbt51_send_cmd(payload, i, true, false);
...
static void query_rsp_handler(uint8_t* data, uint8_t len) {
    ...
    switch (data[1]) {
        case CKBT51_CMD_READ_STATE_REG:
            switch (reg_offset) {
                case 0x05:
                    battery_calculate_voltage(true, data[3] | (data[4] << 8));
                    break;
            }
            reg_offset = 0xFF;
            break;
    ...
    switch (evt_type) {
        ...
        case CKBT51_EVT_BATTERY:
            if (data[0] == 0x01) {
                ...
                battery_calculate_voltage(true, data[1] | (data[2] << 8));
            }
            break;
        default:
